...
%CASIA
lpupilradius = 28; % by trial and error
upupilradius = 75;
lirisradius = 80;
uirisradius = 150;
...
scaling = 0.4;% for samples otherwise computationally expensive
...
circleiris = [row col r];% this gives dimensions

rowd = double(row);
cold = double(col);
rd = double(r); 
% we need to create these circles around the centr of those circles
% identified
irl = round(rowd-rd);
iru = round(rowd+rd);
icl = round(cold-rd);
